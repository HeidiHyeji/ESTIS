...
<div class="title">
<br>
<b>Exchange Student 일괄처리 상세화면</b>
<hr class="titleline">
</div>
<form action="/ESTIS/AdminFSController" method="post" >
<ul><li><strong>업로드 정보</strong></li></ul>
<table class="bluetop">
<tr><th>업로드 일시</th><td><span id="clock"></span></td></tr>
<tr><th>업로드 위치</th><td>FS</td></tr>
<tr><th>처리 운영자</th><td><%= request.getRemoteAddr() %></td></tr>
<tr><th>처리 건수 요약</th><td>엑셀: <%= application.getAttribute("fsvosize") %>건</td></tr>
</table>
<ul><li><strong>업로드 상세 정보</strong></li></ul> 
<table class="bluetop">
<tr><th>번호</th><th>fsid</th><th>pw</th><th>name</th><th>email</th>
<th>language</th><th>sid</th><th>phone</th><th>major</th><th>buddyid</th><th>semester</th></tr>
<%
int size=(Integer)application.getAttribute("fsvosize");
for(int i=0;i<size;i++) {
	FSBean vo=(FSBean)application.getAttribute("fsvo"+i);
%>
<tr><td><%=i+1 %></td><td><%=vo.getFsid() %></td><td><%=vo.getPw() %></td>
	<td><%=vo.getName() %></td><td><%=vo.getEmail() %></td><td><%=vo.getLanguage() %></td>
	<td><%=String.valueOf(vo.getSid())%></td><td><%=vo.getPhone() %></td><td><%=vo.getMajor() %></td>
	<td><%=vo.getBuddyid() %></td><td><%=String.valueOf(vo.getSemester())%></td></tr>
<%} %>
...
<helpcontents>- '일괄처리' 버튼을 클릭하면, Exchange Student 정보의 일괄 처리가 진행됩니다.</helpcontents><br>
